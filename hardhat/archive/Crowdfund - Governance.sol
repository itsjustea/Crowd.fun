...
/**
 * @title Crowdfund - Enhanced with One-Person-One-Vote Governance
 * @notice Decentralized crowdfunding with milestone-based releases and democratic voting
 * @dev Each contributor gets one vote regardless of contribution amount
 */
contract Crowdfund is ReentrancyGuard {
    ...
    address[] public contributors; // NEW: Track all unique contributors
    mapping(address => bool) public isContributor; // NEW: Quick lookup for contributors
    ...
    uint256 public constant APPROVAL_THRESHOLD = 100; // 60% approval required
    uint256 public constant MIN_PARTICIPATION = 100;  // 30% participation required
    ...
    struct Update {
        ...
        string ipfsHash;
        uint256 timestamp;
        uint256 milestoneId; // type(uint256).max for no specific milestone
    }
    ...
    event VoteCast(uint256 indexed milestoneId, address indexed voter, bool support); // Updated: removed weight
    ...
    // ========== GOVERNANCE FUNCTIONS (UPDATED FOR ONE-PERSON-ONE-VOTE) ==========
    
    /**
     * @dev Start voting on milestone completion
     * @notice Each contributor gets exactly one vote, regardless of contribution amount
     */
    ...
    function voteOnMilestone(uint256 _milestoneId, bool _support) external onlyContributor {
        ...
        // UPDATED: Each person gets exactly 1 vote
        if (_support) {
        ...
        emit VoteCast(_milestoneId, msg.sender, _support); // Removed weight parameter
    ...
    function resolveMilestoneVote(uint256 _milestoneId) external {
        ...
        require(block.timestamp > milestoneVotes[_milestoneId].votingDeadline, "Voting still active");
        
        _resolveVote(_milestoneId);
    }
    
    /**
     * @dev Try to auto-resolve vote if threshold is met
    */
    function _tryResolveVote(uint256 _milestoneId) private {
        MilestoneVote storage vote = milestoneVotes[_milestoneId];
        
        uint256 totalVotes = vote.votesFor + vote.votesAgainst;
        uint256 totalContributors = contributors.length;
        
        // Check if we have minimum participation (30% of contributors)
        uint256 minVotes = (totalContributors * MIN_PARTICIPATION) / 100;
        if (totalVotes < minVotes) {
            return; // Not enough participation yet
        }
        
        // Check if approval threshold is met (60% of votes cast)
        uint256 approvalVotes = (totalVotes * APPROVAL_THRESHOLD) / 100;
        
        if (vote.votesFor >= approvalVotes) {
            _resolveVote(_milestoneId);
        }
    }
     ...
     * @dev UPDATED: Uses vote counts instead of ETH amounts
     */
     ...
     * @return votesFor Number of people who voted for
     * @return votesAgainst Number of people who voted against
     * @return votingDeadline Voting deadline timestamp
     * @return resolved Whether vote is resolved
     * @return approved Whether milestone was approved
     * @return hasVoted Whether caller has voted
     * @return voteChoice Caller's vote choice (if voted)
     */
    ...
    /**
     * @dev NEW: Get total number of contributors
     */
    ...
    /**
     * @dev Toggle governance (only before finalization)
     */
